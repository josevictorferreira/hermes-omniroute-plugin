...
One module per Hermes provider capability. Each subclasses a Hermes ABC imported at module top (`agent.image_gen_provider`, `agent.web_search_provider`, `agent.tts_provider`) — those only exist inside a Hermes install.

## THE THREE RETURN CONTRACTS DIFFER (most important gotcha)
...
Web-search uses raw envelopes (no helper fns exist for it); TTS raises instead of returning error dicts. Do not unify these.
...
- **tts.py**: `is_available()` does **not** require a token (unlike the other two) — provider must show in `hermes tools` for setup. Fixed 6-voice OpenAI catalog. `list_models()` keyword-filters `GET /models` (`tts`/`speech`/`audio`). Format clamped to `mp3`/`opus`.
