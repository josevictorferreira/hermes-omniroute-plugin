...
Hermes Agent image-generation backend that routes through
...
```bash
git clone <this-repo> ~/.hermes/plugins/image_gen/omniroute
# or: cp -r . ~/.hermes/plugins/image_gen/omniroute
```
...
export OMNIROUTE_IMAGE_MODEL=...      # optional, overrides config model
```
